...
app.use("/get_charts",async (req,res)=>{
...
try {
      ...
      // for (const chart of user.charts) {
       ...
       let chart = user.charts[0]
        chart.query.unshift(date_filter)

        console.log( JSON.stringify(await collection.aggregate(chart.query).explain('executionStats')));
        const temp = {
          data : await collection.aggregate(chart.query).toArray(),
